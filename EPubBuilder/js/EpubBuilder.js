define(["Construct/DublinCore"], function( DublinCore ) {
    ...
    $.extend(EpubBuilder.prototype, {
        "init": function () {
            var _this = this;
            $.ajax({ async: false, type: "get", dataType: "text", url: "./epub/mimetype", success: function (data) { _this.mimetype = data; } });
            $.ajax({ async: false, type: "get", dataType: "text", url: "./epub/META-INF/container.xml", success: function (data) { _this.container = data; } });
            $.ajax({ async: false, type: "get", dataType: "text", url: "./epub/OEBPS/toc.ncx", success: function (data) { _this.toc = data; } });
            $.ajax({ async: false, type: "get", dataType: "text", url: "./epub/OEBPS/content.opf", success: function (data) { _this.contentOpt = data; } });
            $.ajax({ async: false, type: "get", dataType: "text", url: "./epub/OEBPS/coverpage.html", success: function (data) { _this.coverpage = data; } });
            $.ajax({ async: false, type: "get", dataType: "text", url: "./epub/OEBPS/page.html", success: function (data) { _this.page = data; } });
        },
        ...
        readEpub: function (unzip, setData) {
            ...
            $.each($(elSpine).children(), function (contentOpfSpinIndex, itemref) {
                ...
                var navText = nav ? $(nav).prev("navlabel").text() : (contentOpfSpinIndex === 0 ? "封面" : "");
                var content = unzip.file(_this.toRelativeUrl(OEBPSFolderName + "/" + href)).asText();
                var $content = $(domParser.parseFromString(content, 'text/html'));
                if ($content.find("body").length) $content = $content.find("body");

                var imgs = $content.find("image").add($content.find("img"));
                $.each(imgs, function (i, img) {
                    def = def.then(function () {
                        var _def = $.Deferred();
                        var href = $(img).attr("xlink:href") || $(img).attr("src");
                        var url = OEBPSFolderName + "/" + dir.join("/") + "/" + href;
            ...
            def = def.then(function () {
                var _def = $.Deferred();
                try {
                    var coverItem = $(contentOptXmlDoc).find("meta[name*=cover]").attr("content");
                    var url = $(contentOptXmlDoc).find("item[id*=" + coverItem.split(".")[0] + "]").attr("href");
                    var imageType = _this.getImageType(url);
        ...
        base64toImage: function (content, zipImageFolder) {
            ...
            wrap.find("image").add(wrap.find("img")).each(function (i, e) {
                ...
                if (imageType) {
                    ...
                    // ✅ 修改为 EPUB 内部相对路径 images/
                    $(e).attr("src", "images/" + uuid);
                    if ($(e).closest("svg").length) {
        ...
        exportToEpub: function (options) {
            ...
            try {
                for (var i = 0; i < chapterLength; i++) {
                    ...
                    options.contentArray[i] = options.contentArray[i].replace(/<img [^>]+[^\/](>){1}/gi, function ($0) {
                        var obj = $0.split("");
                        obj.pop();
                        obj.push("/>");
                        return obj.join("");
            ...
            try {
                if (options.coverImage.length) {
                    ...
                    options.coverImage = imgSrc.match(/src="images\/([^"]*)"/)[1];
                }
            } catch (e) {
                options.coverImage = "";
            }
